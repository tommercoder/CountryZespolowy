...
import com.example.country.ActivitiesNav.forElections.adapterAllMayor;
import com.example.country.ActivitiesNav.forElections.model;
import com.example.country.ActivitiesNav.forElections.myadapterMayor;
...
public class all_mayor_votes extends AppCompatActivity {


    RecyclerView recyclerView;
    adapterAllMayor adapter;
    String city, email, passport, password, phone;
    TextView city_name;
    DatabaseReference reff_city;
    String reff_city_id;
    FirebaseDatabase db;
    List<adapterAllMayor> list;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_all_mayor_votes);
        ...
        toolbar.setTitle("All mayor results");
        toolbar.setTitleTextColor(Color.WHITE);
        setSupportActionBar(toolbar);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        recyclerView = (RecyclerView) findViewById(R.id.recyclerViewMayor);
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    ...
                    gov_city_ref.child(passport).addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            ...
                            if(city.equals(snapshot.child("city").getValue().toString())) {//check if city of user equals to city in gov data base
                                ...
                                FirebaseRecyclerOptions<model> options =
                                        new FirebaseRecyclerOptions.Builder<model>()
                                                .setQuery(FirebaseDatabase.getInstance().getReference().child("MayorCandidates").child(city).orderByChild("order"), model.class)
                                                .build();//options for recycle view with reference to candidates

                                adapter = new adapterAllMayor(options);//setting adapter which sets all data to fields like image etc
                            ...
                            {
                                Toast.makeText(all_mayor_votes.this,"Your city is'nt like in your passport\ntry go to support",Toast.LENGTH_LONG).show();
                            }
                        }
