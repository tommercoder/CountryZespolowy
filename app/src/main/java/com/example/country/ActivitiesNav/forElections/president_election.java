        ...
        if (user != null) {
            FirebaseRecyclerOptions<model> options =
                    new FirebaseRecyclerOptions.Builder<model>()
                            .setQuery(FirebaseDatabase.getInstance().getReference().child("PresidentCandidates"), model.class)
                            .build();//options for recycle view with reference to candidates

            adapter = new myadapterPresident(options);//setting adapter which sets all data to fields like image etc
